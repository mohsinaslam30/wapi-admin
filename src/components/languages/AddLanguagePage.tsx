  ...
  return (
    <div className="w-full pb-8">
      ...
      <div className="sticky top-25 z-50 -mx-4 pt-0! sm:-mx-6 lg:-mx-8 px-4 sm:px-6 lg:px-8 bg-light-body-bg dark:bg-(--dark-body) shadow-[0_-55px_0px_0px_var(--light-body-bg)] dark:shadow-[0_-55px_0px_0px_var(--dark-body)] py-4 mb-5 sm:mb-2 flex flex-col sm:flex-row sm:items-center gap-4 transition-all">
        <Button variant="ghost" size="icon" onClick={() => router.back()} className="w-10 h-10 rounded-lg bg-white dark:bg-(--card-color) shadow-sm border border-slate-200 dark:border-(--card-border-color) hover:bg-slate-50 dark:hover:bg-(--dark-sidebar) transition-all">
          <ArrowLeft size={20} />
        </Button>
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-(--text-green-primary) tracking-tight">{isEditMode ? "Edit Language" : "Add New Language"}</h1>
          <p className="text-gray-400 text-sm leading-relaxed">{isEditMode ? "Update language details and translation files." : "Set up a new language and upload localization files."}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit}>
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 dark:bg-(--card-color) dark:border-(--card-border-color) sm:p-6 p-4">
              ...
              <div className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {/* Front JSON */}
                  <div className="space-y-4 flex flex-col">
                    <Label className="text-sm font-medium text-gray-900 dark:text-gray-400">{setting?.app_name || t("common_app_name")} Front JSON</Label>
                      ...
                      <div className="flex flex-col items-center justify-center text-center">
                        ...
                        {frontJsonFileName ? (
                          <div className="space-y-1">
                            <p className="text-xs font-semibold text-gray-900 dark:text-gray-100 truncate max-w-37.5">{frontJsonFileName}</p>
                  ...
                  {/* Admin JSON */}
                  <div className="space-y-4 flex flex-col">
                    <Label className="text-sm font-medium text-gray-900 dark:text-gray-400">{setting?.app_name || t("common_app_name")} Admin JSON</Label>
                    <div className="relative border-2 border-dashed border-gray-200 dark:border-(--card-border-color) rounded-lg p-6 bg-gray-50/50 dark:bg-(--card-color) hover:bg-gray-100/50 dark:hover:bg-(--table-hover) hover:border-(--text-green-primary) transition-all group overflow-hidden">
                      ...
                      <div className="flex flex-col items-center justify-center text-center">
                        ...
                        {adminJsonFileName ? (
                          <div className="space-y-1">
                            <p className="text-xs font-semibold text-gray-900 dark:text-gray-100 truncate max-w-37.5">{adminJsonFileName}</p>
                            <p className="text-[10px] font-medium text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 px-2 py-0.5 rounded-full inline-block">Selected</p>
                          </div>
                        ) : (
                          <div className="space-y-1">
                  ...
                  {/* App JSON */}
                  <div className="space-y-4 flex flex-col">
                    <Label className="text-sm font-medium text-gray-900 dark:text-gray-400">{setting?.app_name || t("common_app_name")} App JSON</Label>
                    <div className="relative border-2 border-dashed border-gray-200 dark:border-(--card-border-color) rounded-lg p-6 bg-gray-50/50 dark:bg-(--card-color) hover:bg-gray-100/50 dark:hover:bg-(--table-hover) hover:border-(--text-green-primary) transition-all group overflow-hidden">
                      ...
                      <div className="flex flex-col items-center justify-center text-center">
                        ...
                        {appJsonFileName ? (
                          <div className="space-y-1">
                            <p className="text-xs font-semibold text-gray-900 dark:text-gray-100 truncate max-w-37.5">{appJsonFileName}</p>
                            <p className="text-[10px] font-medium text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 px-2 py-0.5 rounded-full inline-block">Selected</p>
                          </div>
                        ) : (
                          <div className="space-y-1">
        ...
        <div className="flex items-center justify-end flex-wrap gap-3 mt-8">
          <Button type="button" variant="outline" onClick={() => router.back()} className="px-6 py-5 h-11 border-gray-300 dark:bg-(--card-color) dark:border-(--card-border-color) dark:text-gray-200 shadow-sm dark:hover:bg-(--dark-sidebar) text-gray-700 hover:bg-gray-50 font-medium" disabled={isLoading}>
            Cancel
          </Button>
          <Button type="submit" className="px-6 py-5 h-11 bg-primary hover:bg-primary/90 text-white font-bold rounded-lg shadow-sm transition-all active:scale-95 disabled:opacity-50 gap-2 min-w-40" disabled={isLoading || !name || !locale}>
            {isLoading ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <Check className="w-5 h-5 text-white" />}
            {isLoading ? (isEditMode ? "Saving..." : "Creating...") : isEditMode ? "Save Changes" : "Create Language"}
          </Button>
        </div>
      </form>
    </div>
