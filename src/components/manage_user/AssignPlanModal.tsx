      ...
      <DialogContent className="sm:max-w-lg! max-w-[95vw]! gap-0 bg-white dark:bg-(--card-color) border-none max-h-[90vh] custom-scrollbar overflow-y-auto rounded-xl p-0! shadow-2xl transition-all duration-300">
        ...
        <div className="sm:px-6 px-4 pt-4">
          <div className="relative">
            ...
            <Input placeholder={t("common_search_plans", { defaultValue: "Search plans by name..." })} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-9 h-11 bg-slate-50 dark:bg-page-body border-slate-100 dark:border-(--card-border-color) rounded-xl focus:ring-emerald-500/20 focus:border-(--text-green-primary) transition-all" />
          </div>
        </div>
          ...
          {plansLoading ? (
            ...
            <div className="grid gap-3">
              {filteredPlans.map((plan: Plan) => (
                <div key={plan._id} onClick={() => setSelectedPlanId(plan._id)} className={`relative flex items-center p-4 rounded-lg border-2 cursor-pointer transition-all ${selectedPlanId === plan._id ? "border-(--text-green-primary) bg-emerald-50/50 dark:bg-emerald-900/10" : "border-slate-100 dark:border-(--card-border-color) hover:border-primary/5 dark:hover:border-emerald-800"}`}>
          ...
          <div className="sm:px-6 px-4 pb-4">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 p-4 rounded-xl bg-slate-50 dark:bg-page-body border border-slate-100 dark:border-(--card-border-color)">
              <div className="flex items-center gap-3 flex-1 min-w-0">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-0.5">{t("subscription_duration_label")}</p>
                  <p className="text-xs text-slate-400 dark:text-slate-500">{t("subscription_duration_hint")}</p>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between w-full sm:w-auto gap-4">
                <div className="relative">
                  ...
                  />
                  {durationError && (
                    <p className="absolute -bottom-5 left-0 right-0 text-[9px] text-red-500 font-medium whitespace-nowrap">{durationError}</p>
                  )}
                </div>
                ...
                <div className="text-right shrink-0">
                  ...
                  <p className="text-[10px] text-slate-400 uppercase tracking-wider font-medium">{t("subscription_duration_total")}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <DialogFooter className="sm:p-6 p-4 pt-2 flex flex-col sm:flex-row gap-3">
          <Button type="button" variant="ghost" onClick={handleClose} className="w-full sm:flex-1 h-11 rounded-xl text-slate-600 dark:text-slate-400 bg-slate-100 hover:bg-slate-200 dark:bg-page-body dark:hover:bg-(--table-hover) transition-all order-2 sm:order-1">
            {t("common_cancel")}
          </Button>
          <Button
            type="button"
            ...
            onClick={handleAssign} className="w-full sm:flex-1 h-11 bg-(--text-green-primary) hover:bg-primary text-white rounded-xl font-bold flex items-center justify-center gap-2 transition-all disabled:opacity-50 order-1 sm:order-2">
            {isAssigning ? (
              <>
